SRC_DIR = src/
TEST_DIR = test/
SOURCE_FILES = $(addprefix $(SRC_DIR), stack.c)
HEADER_FILES = $(addprefix $(SRC_DIR), stack.h)
TEST_NAME = stackTest
...
test: $(SOURCE_FILES) $(HEADER_FILES)
	gcc -Wall -lGenericStructures $(SOURCE_FILES) $(TEST_FILE) -o $(TEST_NAME)

debugTest: $(SOURCE_FILES) $(HEADER_FILES)
	gcc -Wall -g -lGenericStructures $(SOURCE_FILES) $(TEST_FILE) -o $(TEST_NAME)
